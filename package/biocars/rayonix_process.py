...
ATTENUATION_FUNCTIONS =\
    {'Ag':, lambda energies: np.exp(-(mu.ElementData(47).sigma)(energies)/12.3)
    'Ti': lambda energies: np.exp(-(mu.ElementData(22).sigma)(energies)/30.)}
...
def glob_npulses(glob_pattern):
    matches = re.findall(r'.*_([0-9]+).*', glob_pattern)
    if len(matches) != 1:
...
def process_and_plot(pattern_list, deconvolution_iterations = 100, plot = True, dtheta = 1e-3, filtsize = 2, center = CENTER, airfactor = 1.0, kaptonfactor = 0.0):
    """
    ...
    def one_spectrum(pattern):
        estimator = full_process(pattern, glob_attenuator, dtheta = dtheta,
            filtsize = filtsize, center = center, airfactor = airfactor,
